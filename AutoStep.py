...
class MiMotion():
    ...
    def login(self, user, password):
        ...
        try:
            ...
            if r1.status_code != 200:
                log_str = f"[登录阶段1] 状态码={r1.status_code} 响应={r1.text}"
                print(f"[登录阶段1] 状态码={r1.status_code} 响应={r1.text}")
                return 0, 0, 0, log_str
            code = r1.json()["access"]
        except Exception as e:
            print("[登录阶段1] 异常:", e)
            try:
                ...
                pass
            print("登录失败")
            log_str = f"[登录阶段1] 异常:{e} 响应内容:{getattr(r1, 'text', '')}"
            return 0, 0, 0, log_str
        ...
        try:
            ...
            if r2.status_code != 200:
                log_str = f"[登录阶段2] 状态码={r2.status_code} 响应={r2.text}"
                print(f"[登录阶段2] 状态码={r2.status_code} 响应={r2.text}")
                return 0, 0, 0, log_str
            info = r2.json()["token_info"]
            return info["login_token"], info["user_id"], info["app_token"], log_str
        except Exception as e:
            print("[登录阶段2] 异常:", e)
            try:
                print("响应内容:", r2.text)
                ...
                pass
            log_str = f"[登录阶段2] 异常:{e} 响应内容:{getattr(r2, 'text', '')}"
            print("获取 token 失败")
            return 0, 0, 0, log_str

    def submitStep(self, user, step, app_token, userid):
    ...
    def main(self, user, password):
        ...
        try:
            ...
            # ---------- 1. 登录 ----------
            login_token, userid, app_token, message = self.login(user, password)
            if 0 in (login_token, userid, app_token):
                print(message)
                return message
            # ---------- 2. 提交步数 ----------
            msg = self.submitStep(user, step, app_token, userid)
            send_message(user, step, msg)
        except Exception as e:
            log_str = f"执行异常:{e}\n"
            print(log_str)
            send_message(user, 0, log_str)
...
if __name__ == "__main__":
    try:
        ...
        openId = ''
        templateId = ''
        if "USER_CONFIG" not in os.environ:
